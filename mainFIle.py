from nonPreemptiveSJF import dataP
...
class RoundRobin:
    ...
    def processData(self, mo_of_processes):
        process_data = []
    ...
    def printData(self, process_data, average_turnaround_time, average_waiting_time, executed_process):
        ...
        for i in range(len(executed_process)):
            print(end = "P" + f'{executed_process[i]}'+' | ')
        print()
...
class Priority:
    ...
    def processData(self, no_of_processes):
        ...
        for i in range(no_of_processes):
            temporary = []
            process_id = int(input("Enter Process ID: P"))

            arrival_time = int(input(f"Enter Arrival Time for P{process_id}: "))

            burst_time = int(input(f"Enter Burst Time for P{process_id}: "))

            priority = int(input(f"Enter Priority for P{process_id}: "))
    ...
    def printData(self, process_data, average_turnaround_time, average_waiting_time, sequence_of_process):
        ...
        for i in range(len(sequence_of_process)):
            print(end="P" + f'{sequence_of_process[i]}' + ' | ')
        print()
...
print("Please choose schedueling algorithm:\n")
print("1. Non-preemptive SJF\n")
print("2. Preempitive Priority\n")
print("3. Round Robin")
try:
    user_choice = int(input("> "))
except:
    print("Please only choose Non-preemptive SJF, Preemptive SJF with Priority, and Round Robin only")

if user_choice == 1:
    try:
        noProcesses = int(input("Enter the number of processes: "))
    #enterProcessID(noProcesses)
    except:
        print("Testing one")
    dataP(noProcesses)
elif user_choice == 2:
